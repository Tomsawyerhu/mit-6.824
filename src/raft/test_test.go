...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("original leader disconnect")
	...
	cfg.connect(leader1)
	DPrintf("original leader connect again")
...
// Entry.  If there is a leader, that leader will fail quickly with a high
// probability (perhaps without committing the Command), or crash after a while
// with low probability (most likey committing the Command).  If the number of
// alive servers isn't enough to form a majority, perhaps start a new server.
// The leader in a new Term may try to finish replicating Entry entries that
